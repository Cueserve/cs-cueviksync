...
- **TECH-STACK.md §5** says app-layer authz is the authority and *"RLS on `tenant_id` is defense-in-depth only."*
- **ARCHITECTURE.md §4** says the opposite — *"the enforcement locus is the database, non-bypassable for user traffic."*
...
Without it, RLS policies and schema changes are applied by hand in the dashboard — unrepeatable, unreviewable — and it undermines PITR/DR, because a restored database has no authoritative schema to restore *to*.
...
> **✅ RESOLVED 2026-07-18.** §6 now states: secrets live in Vercel Environment Variables (app) + Supabase Vault / project secrets (Edge Functions), never committed; the service-role key and any server-only secret MUST NOT use the `NEXT_PUBLIC_` prefix (would inline into the client bundle — only Supabase URL + anon key may be public); anon/service-role keys are dashboard-rotatable via a runbook step on suspected compromise. Chose **Level A** — mechanism + rotate-on-compromise, no fixed cadence (no NFR/compliance obligation requires one per ARCHITECTURE §7; revisit if SOC 2 etc. enters scope). *(Original finding retained below for the record.)*
...
- **`pgmq` + `pg_cron` for the capture buffer** — rejecting SQS/Upstash is right; the durable buffer *in* Postgres matches persist-before-process exactly.
