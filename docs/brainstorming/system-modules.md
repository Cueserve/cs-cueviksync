...
- Why now: PRODUCT.md §4 puts the AI sales assistant in Phase-1 scope, and the §5 success criteria ("teams using the AI assistant"; "90% of flagged cold deals followed up") cannot be met if all AI is deferred. This is the minimal required slice, not the broad AI layer.
...
- Scope note: Phase-1 slice is (a) cold-deal flagging and (b) follow-up / next-action drafting. Cold-deal flagging is on a hybrid path — a deterministic staleness rule at launch (a new tenant has no closed-deal history to train on), upgrading to a learned risk model as outcome data accumulates; labeled honestly per maturity — Phase 1 surfaces it as "auto-flagged" (a staleness rule, not yet AI), with "AI-flagged" reserved for the learned model. The PRODUCT.md §5 metric reads "flagged cold deals" so it does not depend on the label (Product decision, 2026-07-12). AI-drafted quotes and missed-call recovery are in-scope per PRODUCT.md §4 but staged after this slice: quote drafting depends on RFQ/field extraction (a demand-driven future item, not Phase-1 — see ai-features.md §8) and missed-call recovery depends on a telephony connector (Integrations, Later). Human-in-the-loop is absolute: drafts and flags only, no client-facing action sent without explicit approval (PRODUCT.md §6).
- Consequence if excluded: Phase-1 success criteria that name the AI assistant become unachievable.

### Dashboards & Operational Reporting

- Why now: PRODUCT.md §3–§4 commit pipeline and performance reporting to Phase-1 scope; managers need pipeline health, conversion, and cycle-time visibility from launch to coach and forecast. Core operational views ship Must-have and populate as transactional data accumulates.
...
- AI Copilot & Document Intelligence: Defer the broad/advanced AI scope (extraction, enrichment, recommendations, predictive/CV, NL workflows) until core data quality is proven. The minimal AI Sales Assist slice is Must-have (above); only the advanced layer is deferred.
...
| Job Costing — Theoretical vs. Actual Variance | Reject | Out of product (external finance tools) | Estimate-vs-actual cost/margin variance is job costing, out of product per PRODUCT.md §4. Purely-operational signals (e.g. planned vs actual runtime) fold into Dashboards & Operational Reporting (Must-have); the financial variance itself is excluded. |
