...
a **modular monolith** with two supporting runtime roles carved out for the zero-leak
capture path. Three runtime roles share one relational datastore:

1. **Intake Receiver** — a small, public, unauthenticated endpoint that accepts web-form
   submissions, persists each raw payload to the durable intake buffer, and acknowledges
   immediately. It runs no business logic. (PRD-001, NFR-002)
2. **Ingestion Worker** — reads buffered submissions and transforms each into an Inquiry
   record, retrying on failure so nothing is dropped. (PRD-001, NFR-001)
3. **Primary Application** — the modular monolith, exposing a Single-Page Application (SPA)
   over a server JSON Application Programming Interface (API). It holds the domain modules
   and the cross-cutting authorization module, and is the sole authority for authentication,
   Role-Based Access Control (RBAC), and tenant scoping. (PRD-023, PRD-025, NFR-008)
...
- **Intake Receiver** — accepts and durably persists raw web-form submissions; no
  transformation, no authentication. (PRD-001, NFR-002)
- **Intake Buffer** — durable holding area for raw submissions until the worker processes
  them; the persist-before-process point that makes a dropped submission physically hard.
  (NFR-002)
- **Ingestion Worker** — converts buffered submissions into tenant-scoped Inquiry records
  with retry on failure. (PRD-001, NFR-001)
- **Authorization module** — cross-cutting; resolves identity, applies tenant scope, and
  evaluates the static role policy on every API request. (PRD-023, PRD-025, PRD-027, NFR-008)
...
- **Shared Relational Datastore** — one schema holding every tenant's records, configuration,
  per-entity history, and JSON custom-field values, isolated by `tenant_id`. (NFR-006)
...
- **Tenant scope is mandatory and central.** All persistence goes through the data-access
  layer that applies `tenant_id` filtering; no query may bypass it. (PRD-025, NFR-008)
- **Authorize before domain logic.** Every API request passes authentication, the role route
  guard (admin-only configuration is Owner/Admin only), and record-ownership checks before a
  domain module executes. (PRD-023, PRD-026, PRD-027, NFR-008)
...
- **Persist raw before processing.** The Intake Receiver MUST durably store a submission
  before any transformation and MUST NOT run business logic inline. (PRD-001, NFR-002)
...
**Authentication & authorization.** First-party credential authentication precedes every
request (PRD-023). Access is then decided by a centralized, layered pipeline —
**tenant scope → static role guard → record-ownership check** — enforced server-side on every
read and write, so a bypassed client is still denied. (PRD-025, PRD-027, NFR-008)
...
Encryption at rest is a requirement on the managed datastore; the specific mechanism is a
TECH-STACK.md decision.

**Trust boundaries & network exposure.**

- The **Intake Receiver** is the only public, unauthenticated surface; it is isolated from the
  authenticated application so an anonymous flood cannot take the app down, and it only ever
  appends to the durable buffer. (NFR-002, NFR-003)
- The **JSON API** requires an authenticated session for every record operation. (PRD-023)
- The **datastore, buffer, and worker** are internal, reachable only by the application roles.
