...
| GitHub Actions                | Continuous integration on every PR to `main`.                                      | One blocking job gates merges: `lint`, `typecheck`, `format:check`, `test`. No second advisory job — there is no end-to-end suite (§5). A self-discipline net for the solo/process-enforced model (CONTRIBUTING) — the checks run on the real merge state, not just staged files.            |
...
| Playwright / any end-to-end framework                                    | Cut for now. It was in the approved stack and installed in RedyQuote as a devDependency, but neither repo ever gained a config, a spec, or a script — an installed runner that runs nothing implies coverage that does not exist. Removed from both. NFR-012 asks for an automated WCAG 2.1 AA check, which therefore has no runner: an open gap, recorded rather than papered over. Re-adopting it means config, specs, script and CI job in one change, in both repos. |
...
- Continuous integration runs on GitHub Actions for every pull request to `main` and on
  ...
  end-to-end suite (§5). CI complements the local Husky/lint-staged hooks; it does not
  replace the CONTRIBUTING self-review checklist, which remains the human gate. The workflow
  file is byte-identical to RedyQuote's.
