...
- **Person & organization management with duplicate detection** — connected person and
  organization records with a duplicate warning at create time, so relationship data stays
  clean. (PS-2)
...
- As a sales rep, I want the system to warn me when a person or organization looks like a
  duplicate so that I do not create fragmented records.
- As a sales rep, I want to qualify an inquiry into an opportunity linked to a person
  and, optionally, an organization so that a captured lead enters the pipeline with
  context.
...
- **PRD-007** — _**Qualify into opportunity**_ _(Must)_ — The system MUST let a user
  qualify an inquiry into an opportunity, attaching it to a person and, optionally, an
  organization.
...
- **PRD-008** — _**Person and organization records**_ _(Must)_ — The system MUST let a
  user create, view, edit, and delete person and organization records. A person MAY
  exist without an organization. Deleting a record still referenced by an opportunity
  or a job MUST be refused outright, not warned about — the record is moved to the
  Inactive lifecycle status instead (PRD-047). These records are the relationship
  backbone every inquiry and opportunity attaches to.
- **PRD-009** — _**Multi-organization persons**_ _(Should)_ — The system SHOULD let a
  person be associated with more than one organization. A single person often acts
  across several organizations, and each association SHOULD be visible from the person
...
- **PRD-010** — _**Duplicate detection**_ _(Must)_ — The system MUST detect a potential
  duplicate when a user creates a person or organization whose name, email, or phone
  matches an existing record, and warn before saving. Phone is included because a
  person may be recorded with a phone number and no email. The warning lists the
  suspected match and lets the user proceed or cancel. It keeps relationship data clean
  without blocking legitimate entries.
- **PRD-045** — _**Configurable contact lists**_ _(Must)_ — The system MUST let an
  administrator configure the value lists that person and organization records draw
  from: categories, roles, duties, organization-relationship types, and lifecycle
  statuses. Each tenant MUST be seeded with a starting set at provisioning so records
  can be created on day one. A user MUST pick from these lists and MUST NOT invent a
  value mid-workflow, which is what keeps the lists from filling with near-duplicates.
- **PRD-046** — _**List value deactivation and protection**_ _(Must)_ — The system MUST
  let an administrator deactivate a list value, blocking it from new assignment while
  leaving records that already reference it valid and unchanged. Deleting a value still
  in use MUST be refused. Values the system depends on — Prospect, Client, and Primary
  contact — MAY be renamed but MUST NOT be deleted or deactivated.
- **PRD-047** — _**Lifecycle status**_ _(Must)_ — The system MUST let a person or
  organization carry a lifecycle status, and MUST advance it forward only as the
  relationship progresses — qualifying an existing client MUST NOT demote it back to
  prospect. An administrator MAY set a status backwards by hand, and doing so MUST NOT
  prevent it advancing again. Every change MUST be recorded with who made it and when.
- **PRD-048** — _**Person roles and duties in an organization**_ _(Should)_ — The system
  SHOULD let each link between a person and an organization carry one role and zero or
  more duties, both drawn from the configurable lists (PRD-045). An organization MUST
  have a person marked Primary contact at the point an order is qualified for it,
  though not as a standing requirement on every organization record.
...
- **PRD-027** — _**Baseline permission boundaries**_ _(Must)_ — The system MUST enforce
  ...
  - Sales Rep: read/write on opportunities and quotes they own, plus shared inquiry queue
    access for triage and qualification, and full read/write on person and organization
    records — qualification under PRD-007 requires attaching one; cannot access admin
    configuration or role assignment.
  - Office Administrator: create and update inquiry records and related person/organization
  ...
  - Operations: read/write on job records only — dates, status, this-week flag,
    ...
    persons, organizations, opportunities, quotes, or admin configuration.

  Every role reads the contact value lists (PRD-045); only Owner/Admin edits them.
...
- **NFR-010 — Durability.** Committed inquiry, person, opportunity, and quote records
...
| PRD-007        | A user can convert an inquiry into an opportunity and attach an existing or new person and, optionally, an organization; the opportunity links back to the originating inquiry.                                                                                                                                                                                                                                                                                                                                                             |
...
| PRD-008        | A user can create, view, edit, and delete a person and an organization, and can create a person with no organization; deleting a record still referenced by an opportunity or a job is refused, and the record can be set to the Inactive lifecycle status instead.                                                                                                                                                                                                                                                                         |
| PRD-009        | A person can be linked to two or more organizations and each association is visible from the person record.                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| PRD-010        | Creating a person or organization whose name, email, or phone matches an existing record shows a duplicate warning listing the suspected match; the user can proceed or cancel.                                                                                                                                                                                                                                                                                                                                                             |
| PRD-045        | An administrator can add, rename, and reorder values in each of the five contact lists; a newly provisioned tenant already has a usable starting set; a user creating a record picks from the list and cannot type a new value.                                                                                                                                                                                                                                                                                                             |
| PRD-046        | A deactivated list value cannot be chosen on a new record while records already referencing it stay valid; deleting a value still in use is refused; Prospect, Client, and Primary contact can be renamed but neither deleted nor deactivated.                                                                                                                                                                                                                                                                                              |
